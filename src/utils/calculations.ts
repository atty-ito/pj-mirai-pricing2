...
export function computeCalc(data: Data): CalcResult {
  ...
  for (const w of data.workItems) {
    ...
    lineItems.push({
      ...
      note: [
        `サイズ:${w.sizeClass}`,
        `色:${w.colorMode === "mono" ? "モノクロ" : w.colorMode === "gray" ? "グレースケール" : "カラー"}`,
        `解像度:${w.dpi}dpi`,
        `形式:${w.formats.length > 0 ? w.formats.join(" / ") : "未指定"}`,
        w.ocr ? "OCR:あり" : "OCR:なし",
        `メタデータ:${w.metadataLevel === "none" ? "なし" : w.metadataLevel === "basic" ? "基本" : "充実"}`,
        `取扱:${w.handling === "normal" ? "通常" : w.handling === "fragile" ? "脆弱・破損懸念" : w.handling === "bound" ? "製本" : w.handling === "mylars" ? "マイラー" : "混在"}`,
      ].join(" / "),
    });
  }
