...
/**
 * 元の App.tsx にあった buildSpecSections 関数を完全に再現
 * 仕様プロファイル(standard/ndl/gunma)に応じた詳細な日本語規定文を一切省略せず保持
 */
export function buildSpecSections(data: Data): SpecSection[] {
  ...
  const workItemSummary = data.workItems
    .map(
      (w) =>
        `- ${w.title}：数量 ${num(w.qty)} ${w.unit}／${w.sizeClass}／${w.colorMode === "mono" ? "モノクロ" : w.colorMode === "gray" ? "グレースケール" : "カラー"}／${w.dpi}dpi／形式 ${w.formats.join("・")}／OCR ${w.ocr ? "有" : "無"}／メタデータ ${w.metadataLevel === "none" ? "なし" : w.metadataLevel === "basic" ? "基本" : "充実"}／取扱 ${w.handling === "normal" ? "通常" : w.handling === "fragile" ? "脆弱・破損懸念" : w.handling === "bound" ? "製本" : w.handling === "mylars" ? "マイラー" : "混在"}`
    )
    .join("\n");
  ...
  push("1. 目的・適用範囲", [
    ...
    `仕様レベルは「${profile === "standard" ? "標準" : profile === "ndl" ? "詳細" : "厳格"}」である。仕様レベルに応じて、記述の粒度、検査強度、媒体要件、メタデータ要件が変化する。`,
  ...
  if (profile !== "standard") {
    mdLines.push("作業・機材情報（例：ホストPC、OS、スキャナ／カメラ、画像処理ソフト）は、所定の形式で記録する。");
  }
  ...
  if (profile !== "standard") {
    logLines.push(
      "詳細以上では、以下の類型をログで明示する：スキャニング対象外、切断、サイズ変更、分割、誤り、ページ貼り付き等。また、資料群ごとの整理情報、フォルダ構成、媒体ラベル情報等を含める。"
    );
  }
  ...
  // 10. 検査・是正
  const inspLines = [
    `検査レベルは「${data.inspectionLevel}」である。`,
    "検査は、(i) 画質、(ii) 欠落・重複、(iii) 命名・フォルダ整合、(iv) メタデータ整合、(v) 媒体要件の順に確認する。",
  ];
  if (data.inspectionLevel === "full" || data.inspectionLevel === "double_full") {
  ...
  // 11. 納品媒体・保管
  const mediaLines = ["納品は、成果物を所定のフォルダ構成で格納し、媒体単位で管理する。"];
  if (profile !== "standard") {
    mediaLines.push(
      "詳細以上では、外付け媒体等への格納、媒体ラベル、ウイルスチェック、チェックサム等の運用を行う。",
  ...
  if (profile === "gunma") {
    secLines.push("厳格では、アクセス制御、作業者権限、持出制限、監査可能なログ等により、追跡可能性を担保する。");
  }
